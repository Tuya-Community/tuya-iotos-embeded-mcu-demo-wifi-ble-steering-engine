...
void all_data_update(void)
{
    ...
    mcu_dp_bool_update(DPID_SWTICH,当前开关); //BOOL型数据上报;
    mcu_dp_value_update(DPID_LEVEL,当前level); //VALUE型数据上报;
...
******************************************************************************/
/*****************************************************************************
函数名称 : dp_download_swtich_handle
功能描述 : 针对DPID_SWTICH的处理函数
输入参数 : value:数据源数据
        : length:数据长度
...
static unsigned char dp_download_level_handle(const unsigned char value[], unsigned short length)
{
    ...
    level = mcu_get_dp_download_value(value,length);
		T_servo=level;   
    //处理完DP数据后应有反馈
    ret = mcu_dp_value_update(DPID_LEVEL,level);
    if(ret == SUCCESS)
        return SUCCESS;
    else
        return ERROR;
